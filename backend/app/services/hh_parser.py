...
class HeadHunterParser:
        ...
        try:
            async with self.session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    logger.error(f"❌ HeadHunter 403: Запрос заблокирован. IP: {response.real_url.host}")
                    logger.error("💡 Решение: Используйте прокси или VPN для доступа к HeadHunter с Render")
                    return []
                else:
                    logger.error(f"HeadHunter API ошибка: {response.status}")
                    return []
        except aiohttp.ClientError as e:
            logger.error(f"Ошибка сети при запросе к HeadHunter: {e}")
            return []
    ...
    def _detect_category(self, title: str, description: str) -> str:
        ...
        categories = {
            ...
            "design": ["дизайнер", "designer", "ui", "ux", "графический", "веб-дизайн", "figma", "photoshop"],
            "smm": ["smm", "маркетолог", "маркетинг", "social media", "таргетолог", "instagram", "facebook"],
            "video": ["видео", "video", "монтаж", "оператор", "режиссер", "premiere", "after effects"],
            "copywriting": ["копирайтер", "copywriter", "редактор", "журналист", "контент", "текст"],
            "analytics": ["аналитик", "analyst", "data", "big data", "sql", "tableau", "power bi"],
            "project_management": ["менеджер проектов", "project manager", "scrum", "agile", "product owner"],
            "quality_assurance": ["qa", "тестировщик", "tester", "quality assurance", "selenium", "pytest"],
            "devops": ["devops", "sysadmin", "системный администратор", "docker", "kubernetes", "aws"],
